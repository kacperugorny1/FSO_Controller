...
struct ac_struct {
    ...
    // 8-bit/Enum variables
    ac_states state;
    uint8_t steps;
    uint8_t pavg_num;       // Retained: Assuming used in other states
    uint8_t SMA_num;        // Retained: Assuming used in other states
    uint8_t EMA_num;
    uint8_t iter;

    // Boolean flags (1-byte each)
    bool dir;               // Tracks active axis (e.g., 1 for Z, 0 for Y)
    bool dirz;              // Physical pin state for Z
    bool diry;              // Physical pin state for Y
    bool me_init;

    // --- New Sweep Logic Variables ---
    bool histeresis_hit;
    bool first_sweep;
    bool second_sweep;
    bool end_on_max;
...
//	HAL_Delay(1);
	...
	if((opt.optimize_state_machine != OPT_IDLE || ac.state == AC_RUN) && !stepsRemainingY && !stepsRemainingZ){
		HAL_Delay(50);
	}
	...
	switch (c) {
		...
		case '\'':
			if(ac.state == AC_IDLE) {
			  ...
			  ac.me_init = false;
			  // Inside AC_IDLE right before switching to AC_RUN:
			  ac.dir = 1;
			  ac.rx_raw_max = 0;
			  ...
			  // Optional but recommended: reset the moving average to the current reading
			  // so the EMA doesn't have to "climb up" from 0 on the very first few ticks.
			  ac.mov_avg = rx_raw;
